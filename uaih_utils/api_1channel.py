...
# 请求的接口名 这个版本特化单声道处理，会返回不同说话人的语言，准确性有待考量
# rjw@24.7.4
api_upload = '/upload'
...
class RequestApi(object):
    ...
    def get_result(self):
        ...
        for i in lattice:
            ...
            if json_1best['st']['rl'] == '1':
                
                for i in json_1best['st']['rt']:
                    for j in i['ws']:
                        all_text_machine.append(j['cw'][0]['w'])
            else:
                for i in json_1best['st']['rt']:
                    for j in i['ws']:
                        all_text_customer.append(j['cw'][0]['w'])
        all_text_c = ''.join(all_text_customer)
        all_text_m = ''.join(all_text_machine)

        return all_text_c,all_text_m
...
if __name__ == '__main__':
    folder_path = 'dataset/mp3_1chan'  # 音频文件夹路径
    wav_files = collect_and_convert_audios(folder_path)  # 转换音频文件为.wav格式

    num_files_to_test = 3  # 要测试的文件数量
    wav_files_to_test = wav_files[:num_files_to_test]  # 选择前 num_files_to_test 个文件

    with open('Output-cn-7.4.txt', 'a+', encoding='utf-8') as f, ThreadPoolExecutor(max_workers=50) as executor:
        futures = {executor.submit(RequestApi(appid="6ec56b9d",
                                              secret_key="41c5959915dc37c7f35a309c66b047d8",
                                              upload_file_path=i).get_result): i for i in wav_files_to_test}
